...
# 1단계: 데이터 로드 및 탐색
print("="*70)
print("Gradient Boosting 모델 - 학생 난이도 진단 시스템")
print("="*70)
...
# 3단계: Gradient Boosting 모델 학습
print("\n" + "="*70)
print("3단계: Gradient Boosting 모델 학습")
print("-"*70)
...
print(f"\n모델 파라미터:")
print(f"   n_estimators: 100 (부스팅 단계 수)")
print(f"   learning_rate: 0.05 (학습률)")
print(f"   max_depth: 3")
print(f"   min_samples_split: 2")
print(f"   min_samples_leaf: 1")
print(f"   subsample: 0.8 (각 반복마다 사용할 샘플 비율)")
...
print(f"\n학습된 그래디언트 부스팅의 특성:")
print(f"   부스팅 단계 수: {gb_model.n_estimators}")
print(f"   각 나무의 최대 깊이: {gb_model.max_depth}")
print(f"   학습률: {gb_model.learning_rate}")
...
fig.suptitle('Gradient Boosting 모델 성능 분석', fontsize=16, fontweight='bold')
...
plt.tight_layout()
plt.savefig('gradient_boosting_performance.png', dpi=300, bbox_inches='tight')
print(f"\n성능 시각화 저장: gradient_boosting_performance.png")
plt.show()
...
print(f"\n부스팅 과정에서의 정확도 변화:")
print(f"   초기 (1번째 반복): Train={train_scores[0]:.4f}, Test={test_scores[0]:.4f}")
print(f"   중간 (50번째 반복): Train={train_scores[49]:.4f}, Test={test_scores[49]:.4f}")
print(f"   최종 (100번째 반복): Train={train_scores[-1]:.4f}, Test={test_scores[-1]:.4f}")

# 부스팅 과정 시각화
plt.figure(figsize=(10, 5))
plt.plot(range(1, len(train_scores)+1), train_scores, label='학습 데이터', marker='o', markersize=3, linewidth=2)
plt.plot(range(1, len(test_scores)+1), test_scores, label='테스트 데이터', marker='s', markersize=3, linewidth=2)
plt.xlabel('부스팅 단계', fontweight='bold')
plt.ylabel('정확도 (Accuracy)', fontweight='bold')
plt.title('Gradient Boosting 학습 과정', fontweight='bold')
plt.legend(fontsize=11)
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('gradient_boosting_learning_curve.png', dpi=300, bbox_inches='tight')
print(f"학습 곡선 저장: gradient_boosting_learning_curve.png")
plt.show()
...
predictions = gb_model.predict(new_student_data)
probabilities = gb_model.predict_proba(new_student_data)
