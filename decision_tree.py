...
# 1단계: 데이터 로드 및 탐색
print("="*70)
print("Decision Tree 모델 - 학생 난이도 진단 시스템")
print("="*70)
...
# 3단계: Decision Tree 모델 학습
print("\n" + "="*70)
print("3단계: Decision Tree 모델 학습")
print("-"*70)
...
print(f"\n학습된 의사결정 나무의 특성:")
print(f"   트리의 깊이: {dt_model.get_depth()}")
print(f"   리프 노드 개수: {dt_model.get_n_leaves()}")
...
fig.suptitle('Decision Tree 모델 성능 분석', fontsize=16, fontweight='bold')
...
print(f"\n성능 시각화 저장: decision_tree_performance.png")
plt.show()
...
predictions = dt_model.predict(new_student_data)
probabilities = dt_model.predict_proba(new_student_data)
